...
class MorphingLander(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        if self.randomize:
            self.cfg["sim"]["dt"] = np.random.uniform(0.001, 0.015)
        ...
        self.tau = self.cfg["task"]["motor"]["tau"]
        ...
        # Covert ori to euler angles from quat :
        ...
        self.actions = torch.zeros((self.num_envs, 5), device=self.device, dtype=torch.float32, requires_grad=False)
    ...
    def reset_idx(self, env_idx):
        ...
        if self.randomize:
            ...
            self.root_states[env_idx, 2] = torch_rand_float(0.5, 3, (num_reset, 1), device=self.device).flatten()
    ...
    def pre_physics_step(self, _actions):
        ...
        new_actions = self.postprocess_actions(actions)
        ...
        tilt_actions[:, 0] = new_actions[:, 4]
        tilt_actions[:, 3] = new_actions[:, 4]
        if self.randomize:
    ...
    def postprocess_actions(self, actions):
        ...
        new_actions[:, :4] = (new_actions[:, :4] + 1) / 2
        # new_actions[:, :4] = self.first_order_filter(self.actions[:, :4], new_actions[:, :4])

        # If actions[:, 4] between 0.5 and -0.5, set to 0, if lower than -0.5, set to -1 if higher than 0.5 set to 1 :
        new_actions[:, 4] = torch.where(new_actions[:, 4] > 0.25, 0.95, torch.where(new_actions[:, 4] < -0.25, -0.95, 0))
        return new_actions
    
    def first_order_filter(self, x, y):
        dt = torch.tensor(self.dt, device=self.device)
        tau = torch.tensor(self.tau, device=self.device)
...
def compute_morphing_lander_reward(root_pos, root_ori, root_ori_euler, root_lin_vel, root_ang_vel, dof_pos, reset_buf, progress_buf, thrusts, terminal_cost_added, contact_forces, init_pos, max_episode_length):
    ...
    reward = 2*z_reward + 0.75*xy_reward + up_reward + spinnage_reward + 6*tilt_reward + 1.25*landing_reward
